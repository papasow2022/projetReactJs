...
const Catalogue = () => {
  ...
  // Utiliser les vrais produits du contexte
  const { allProducts, loading } = useProducts();
  
  // Filtrage produits selon la taille sélectionnée et les badges
  const filteredProducts = allProducts.filter(p => {
    if (selectedSizes.length > 0 && p.sizes && !p.sizes.some(size => selectedSizes.includes(size))) return false;
    if (selectedColors.length > 0 && p.colors && !p.colors.some(c => selectedColors.includes(c.code))) return false;
    if (selectedCategories.length > 0 && !selectedCategories.includes(p.category)) return false;
    if (selectedRatings.length > 0 && (!p.rating || !selectedRatings.includes(Math.floor(p.rating)))) return false;
  ...
  return (
    <>
      <div className="container-fluid py-4">
        {/* Filtres avancés */}
        <div className="mb-4 p-3 bg-white rounded shadow-sm d-flex flex-wrap gap-3 align-items-end">
          <div>
            <label>Catégorie</label>
            <select multiple value={selectedCategories} onChange={e => setSelectedCategories(Array.from(e.target.selectedOptions, o => o.value))} className="form-select">
          ...
          <div>
            ...
            <div className="d-flex gap-1 flex-wrap">
              {mockColors.map(color => (
                <span key={color} onClick={() => setSelectedColors(selectedColors.includes(color) ? selectedColors.filter(c => c !== color) : [...selectedColors, color])} style={{ width: 22, height: 22, borderRadius: '50%', background: color, border: selectedColors.includes(color) ? '2px solid #232f3e' : '1px solid #ccc', cursor: 'pointer', display: 'inline-block' }} title={color}></span>
              ))}
            </div>
          </div>
          <div>
            <label>Taille</label>
            <select multiple value={selectedSizes} onChange={e => setSelectedSizes(Array.from(e.target.selectedOptions, o => o.value))} className="form-select">
              {mockSizes.map(size => <option key={size} value={size}>{size}</option>)}
            </select>
          </div>
          <div>
            <label>Note</label>
            <select multiple value={selectedRatings} onChange={e => setSelectedRatings(Array.from(e.target.selectedOptions, o => parseInt(o.value)))} className="form-select">
              {[5,4,3,2,1].map(r => <option key={r} value={r}>{r} étoiles et +</option>)}
            </select>
          </div>
          <div>
            <label>Prix (€)</label>
            <div className="d-flex align-items-center gap-2">
              <input type="number" min={0} max={priceRange[1]} value={priceRange[0]} onChange={e => setPriceRange([parseInt(e.target.value)||0, priceRange[1]])} style={{ width: 60 }} />
              <span>-</span>
              <input type="number" min={priceRange[0]} value={priceRange[1]} onChange={e => setPriceRange([priceRange[0], parseInt(e.target.value)||priceRange[1]])} style={{ width: 60 }} />
            </div>
          </div>
          <div>
