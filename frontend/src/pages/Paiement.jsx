    ...
    try {
      ...
      const orderData = {
        customer: {
          ...
          lastName: user.nom || '',
          email: user.email,
          phone: adresse.telephone,
          address: {
            street: adresse.adresse,
            city: adresse.ville,
            postalCode: adresse.codePostal || '001',
            country: adresse.pays || 'Guinée'
          }
        },
        items: items.map(item => ({
          ...
          quantity: item.qty || item.qte,
          image: item.image,
          // Informations enrichies pour le backend (utiliser les vraies valeurs du panier)
          brand: item.brand || 'Marque inconnue',
          category: 'chaussure', // Toujours chaussure pour les chaussures
          genre: item.genre || item.category || 'homme',
          color: item.color || 'Non spécifié',
          size: item.size || 'N/A'
        })),
        shipping: 0,
        tax: 0,
        notes: `Mode de paiement: ${modePaiement}`,
        source: 'website'
      };

      console.log('📤 Envoi de la commande au backend...', orderData);
      
      // Créer la commande au backend
      const backendResponse = await createOrder(orderData);
      
      if (backendResponse.success) {
        console.log('✅ Commande créée au backend:', backendResponse.order);
        
        // Générer un numéro de commande local pour compatibilité
        const localId = backendResponse.order.orderNumber || 'CMD' + Math.floor(100000 + Math.random() * 900000);
        ...
        // Nouvelle commande locale pour compatibilité
        const nouvelleCommande = {
          id: localId,
          _id: backendResponse.order.id,
        ...
        console.log('📋 Commande locale créée:', nouvelleCommande);
        ...
        if (user && user.email) {
          ...
          console.log('💾 Commande sauvegardée localement pour:', user.email);
        }
        ...
        throw new Error(backendResponse.message || 'Erreur lors de la création de la commande');
      }
      
    } catch (error) {
      console.error('❌ Erreur lors de la validation de la commande:', error);
      setError(t("order_validation_error") + ': ' + error.message);
    }
  };
                      ...
                      onClick={() => {
                        ...
                        if (result.ok) {
                          ...
                          if (result.reason === 'NOT_FOUND') {
                            ...
                            setGiftCardError('Carte cadeau expirée');
                          } else if (result.reason === 'ALREADY_REDEEMED') {
                            setGiftCardError('Carte cadeau déjà utilisée');
                          } else if (result.reason === 'NO_BALANCE') {
                            setGiftCardError('Carte cadeau sans solde');
                          } else {
                            setGiftCardError('Erreur: ' + result.reason);
                          }
                          setGiftCardAmount(0);
                        }
                      }}
