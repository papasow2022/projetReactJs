...
export default function AdminDashboard() {
  ...
  const { dashboardStats, loading, error, loadDashboardStats } = useAdmin();
  ...
  useEffect(() => {
    if (dashboardStats) {
      setStats(prev => ({
        ...prev,
        totalVendors: dashboardStats.users.vendorUsers || 0,
        activeVendors: dashboardStats.users.validatedVendors || 0,
        pendingVendors: dashboardStats.users.pendingVendors || 0,
        totalProducts: dashboardStats.products.totalProducts || 0,
        pendingProducts: dashboardStats.products.pendingProducts || 0,
        totalOrders: dashboardStats.orders.totalOrders || 0,
        totalRevenue: dashboardStats.orders.totalRevenue || 0,
        totalReviews: 0, // À implémenter si nécessaire
        pendingReviews: 0, // À implémenter si nécessaire
        totalCommissions: dashboardStats.transactions.totalFees || 0
      }));

      // Mettre à jour l'activité récente
      if (dashboardStats.recentActivity) {
        setRecentActivity(dashboardStats.recentActivity.slice(0, 10));
      }
      ...
      setNotifications([
        { 
          id: 1, 
          message: `${dashboardStats.users.pendingVendors || 0} vendeurs en attente de validation`, 
          type: (dashboardStats.users.pendingVendors || 0) > 0 ? 'warning' : 'info', 
          time: 'Maintenant' 
        },
        { 
          id: 2, 
          message: `${dashboardStats.products.pendingProducts || 0} produits en attente de modération`, 
          type: (dashboardStats.products.pendingProducts || 0) > 0 ? 'warning' : 'info', 
          time: 'Maintenant' 
        },
        { 
          id: 3, 
          message: `${dashboardStats.orders.totalOrders || 0} commandes totales`, 
          type: 'info', 
          time: 'Maintenant' 
        },
        { 
          id: 4, 
          message: `${dashboardStats.support.openTickets || 0} tickets de support ouverts`, 
          type: (dashboardStats.support.openTickets || 0) > 0 ? 'danger' : 'info', 
          time: 'Maintenant' 
        }
      ]);
    }
  ...
  const quickActions = [
    ...
    { title: 'Gérer les stocks', icon: BiBarChart, link: '/admin/stocks', color: 'info' },
    { title: 'Gérer les commandes', icon: BiShoppingBag, link: '/admin/orders', color: 'success' },
    { title: 'Retours & Remboursements', icon: BiCheckCircle, link: '/admin/returns', color: 'warning' },
