...
export default function ProductDetail() {
  ...
  useEffect(() => {
    if (clickedImage && products && products.length > 0) {
      ...
      // Si toujours rien trouvé, essayer de récupérer les données depuis l'API des images
      if (!foundProduct && clickedImage) {
        // Fonction async pour récupérer les données
        const fetchImageData = async () => {
          try {
            const category = clickedImage.includes('/homme/') ? 'homme' : 
                           clickedImage.includes('/enfant/') ? 'enfant' : 'femme';
            
            // Récupérer les données de l'image depuis l'API
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/${category}`);
            if (response.ok) {
              const images = await response.json();
              const imageData = images.find(img => img.path === clickedImage);
            
              if (imageData) {
                foundProduct = {
                  id: `image-${imageData.path.replace(/[^a-zA-Z0-9]/g, '-')}`,
                  slug: imageData.alt?.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'produit',
                  name: imageData.name || imageData.alt || imageData.model || 'Produit',
                  image: clickedImage,
                  price: imageData.price || (category === 'enfant' ? 200000 : 250000),
        ...
        // Fallback: créer un produit synthétique si l'API échoue
        if (!foundProduct) {
          ...
          <div className="col-md-5">
            ...
            <StockManager 
              ...
              onStockChange={(stockInfo) => {
                ...
                console.log('Stock mis à jour:', stockInfo);
              }}
            />
            <div className="mt-4">
