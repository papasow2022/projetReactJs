...
export default function AdminSupport() {
  ...
  const [useApi, setUseApi] = useState(true); // Toggle API/LocalStorage
  ...
  const loadTickets = async () => {
    ...
    try {
      if (useApi) {
        ...
        console.log('✅ Tickets chargés via API:', response.data.tickets?.length || 0);
      } else {
        // Utiliser le localStorage (fallback)
        const storedTickets = getTickets();
        setTickets(storedTickets);
        console.log('✅ Tickets chargés via localStorage:', storedTickets.length);
      }
    } catch (error) {
  ...
  const handleUpdateTicket = async () => {
    try {
      if (useApi) {
        // Utiliser l'API backend
        const response = await supportService.updateTicket(editingTicket, formData);
        const updatedTicket = response.data;
        setTickets(tickets.map(ticket => 
          ticket._id === editingTicket ? updatedTicket : ticket
        ));
        console.log('✅ Ticket mis à jour via API:', updatedTicket);
      } else {
        // Utiliser le localStorage (fallback)
  ...
  const handleSendResponse = async () => {
    ...
    try {
      if (useApi) {
        // Utiliser l'API backend
        const response = await supportService.addConversation(selectedTicket._id, newConversation);
        const updatedTicket = response.data;
        ...
        console.log('✅ Conversation ajoutée via API:', newConversation);
        
        // Notifier le client de la réponse admin
        supportNotificationService.notifyClientTicketReply(selectedTicket, newConversation);
      } else {
        // Utiliser le localStorage (fallback)
        ...
        if (updatedTicket) {
          ...
          // Notifier le client de la réponse admin (simulation)
          supportNotificationService.notifyClientTicketReply(selectedTicket, newConversation);
        }
      }
      
      // Envoyer via le chat temps réel
      realtimeChat.sendMessage(selectedTicket._id || selectedTicket.id, newConversation);
  ...
  const filtered = tickets.filter(t => {
    const ticketId = t._id || t.id; // Support API (_id) et localStorage (id)
    const matchesSearch = `${ticketId} ${t.subject} ${t.user} ${t.email}`.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="d-flex gap-2">
          <div className="form-check form-switch">
            ...
            <label className="form-check-label" htmlFor="apiToggle">
              {useApi ? 'API Backend' : 'LocalStorage'}
            </label>
          </div>
