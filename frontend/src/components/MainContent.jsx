...
export default function MainContent() {
  ...
  const settings = {
    ...
    slidesToScroll: 1,
    pauseOnHover: false,
    // Amélioration de l'accessibilité pour éviter les conflits aria-hidden
    accessibility: true,
    focusOnSelect: false,
    focusOnChange: false,
    swipeToSlide: true,
    adaptiveHeight: false,
    // Désactiver le focus automatique sur les slides non actives
    focusOnChange: false,
    // Empêcher le focus sur les éléments des slides cachées
    beforeChange: (current, next) => {
      // Retirer le focus des éléments de la slide actuelle
      const currentSlide = document.querySelector(`.slick-slide[data-index="${current}"]`);
      if (currentSlide) {
        const focusableElements = currentSlide.querySelectorAll('button, a, input, select, textarea, [tabindex]:not([tabindex="-1"])');
        focusableElements.forEach(el => el.setAttribute('tabindex', '-1'));
      }
    },
    afterChange: (current) => {
      // Restaurer le focus sur les éléments de la slide active
      const activeSlide = document.querySelector(`.slick-slide[data-index="${current}"]`);
      if (activeSlide) {
        const focusableElements = activeSlide.querySelectorAll('button, a, input, select, textarea, [tabindex="-1"]');
